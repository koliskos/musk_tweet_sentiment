...
y_predicted<- predict(crossv_glmnet_fit, s=crossv_glmnet_fit$lambda.min, newx = as.matrix(sentis_x))
y_predicted
summary(sentis_y) # mean is -11.507
sst<- sum((sentis_y--11.507)^2)
sse <- sum((y_predicted - sentis_y)^2)
r_sq <- 1 - sse/sst
r_sq

tLL <- crossv_glmnet_fit$nulldev - deviance(crossv_glmnet_fit)
k <- crossv_glmnet_fit$df
n <- crossv_glmnet_fit$nobs
